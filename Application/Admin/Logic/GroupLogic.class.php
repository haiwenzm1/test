...
class GroupLogic extends Model {
    ...
    public function getAllInfo(){
        ...
        return array('code'=> 1, 'msg'=>'获取信息成功', 'info'=> $result);
    }
    ...
    public function addInfo($data){
        $map = array();
        $map['id'] = $data['pid'];
        $roleid = D('Group')->getRoleidById($map);
        if($roleid[0]['roleid']){
            $datas = array();
            $datas['pid'] = $data['pid'];
            ...
            return array('code'=> 1, 'msg'=>'插入成功', 'info'=> $result);
        }else{
    ...
    public function updateInfoById($data){
        $map = array();
        $map['id'] = $data['id'];
        $version = D('Group')->getVersionById($map);
        if($version[0]['version'] == $data['version']){
            ...
            return array('code'=> 1, 'msg'=>'更新成功', 'info'=> $result);
        }else{
    ...
    public function deleteInfoById($data){
        ...
        if($version[0]['version'] == $data['version']){
            ...
            $map['update_time'] = time();
            $map['version'] = intval($data['version'])+1;
            $result = D('Group')->deleteInfoById($map);
            return array('code'=> 1, 'msg'=>'删除成功', 'info'=> $result);
        }else{
            return array('code'=> 0, 'msg'=>'请刷新重试', 'info'=> '');
        }
    }
